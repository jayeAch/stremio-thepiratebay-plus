...
const defaultProxies = process.env.PROXIES ? process.env.PROXIES.split(',') : ['https://piratebay.live'];
...
function search(keyword, config = {}, retries = 2) {
  ...
  return raceFirstSuccessful(proxyList
      ...
      .catch((err) => {
        console.error(`Search attempt failed for "${keyword}" (retries left: ${retries - 1}):`, err.message || err);
        return search(keyword, config, retries - 1);
      });
}
...
$('table[id=\'searchResult\'] tr').each(function() {
  const magnetHref = $(this).find('a[title=\'Download this torrent using magnet\']').attr('href');
  if (!magnetHref) return;

  const torrent = {
    name: $(this).find('.detLink').text(),
    seeders: parseInt($(this).find('td[align=\'right\']').eq(0).text(), 10),
    infoHash: decode(magnetHref).infoHash
  };
